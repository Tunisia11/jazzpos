...
class _ProductEditScreenState extends ConsumerState<ProductEditScreen> {
  ...
  Future<void> _loadData() async {
    ...
    if (widget.productId != null) {
      final product = await (db.select(db.products)..where((tbl) => tbl.id.equals(widget.productId!))).getSingleOrNull();
      if (product != null) {
        _nameCtrl.text = product.name;
        _secondaryNameCtrl.text = product.secondaryName ?? '';
        _descriptionCtrl.text = product.description ?? '';
        _selectedCategoryId = product.categoryId;
        _defaultCostCtrl.text = Money.fromMillimes(product.defaultCostMillimes).format(includeCurrency: false, useGrouping: false);
        _defaultPriceCtrl.text = Money.fromMillimes(product.defaultPriceMillimes).format(includeCurrency: false, useGrouping: false);
  ...
  Future<void> _saveProduct() async {
    ...
    if (_variants.isEmpty || _variants.where((v) => v.isEnabled).isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Veuillez générer au moins une variante active pour ce vêtement'),
    ...
    try {
      ...
      await catalogService.createProductWithMatrix(
        ...
        secondaryName: _secondaryNameCtrl.text.trim().isNotEmpty ? _secondaryNameCtrl.text.trim() : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppTheme.background,
      appBar: AppBar(
        title: Text(widget.productId == null ? 'Nouvel Article Vêtement' : 'Modifier Article'),
        ...
        actions: [
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_isSaving ? 'ENREGISTREMENT...' : 'ENREGISTRER L\'ARTICLE'),
            ),
          ),
        ],
      ),
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Informations Générales de l\'Article', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ...
                      children: [
                        Expanded(
                          ...
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: 'Désignation de l\'article *', hintText: 'ex: Chemise Slim Fit'),
                        ...
                        Expanded(
                          ...
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: 'Nom secondaire / Arabe (optionnel)'),
                          ),
                        ),
                        const SizedBox(width: 16),
                        Expanded(
                          ...
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: 'Code / Préfixe SKU', hintText: 'ex: CHM-SLIM'),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: 'Coût d\'Achat Défaut (TND)', suffixText: 'TND'),
                            onChanged: (_) => setState(() {}),
                          ),
                        ),
                        const SizedBox(width: 16),
                        Expanded(
                          child: TextFormField(
                            ...
                            keyboardType: TextInputType.number,
                            decoration: const InputDecoration(labelText: 'Prix de Vente Défaut (TND) *', suffixText: 'TND'),
                            validator: (val) => val == null || val.trim().isEmpty ? 'Champ requis' : null,
                            onChanged: (_) => setState(() {}),
                          ),
                        ),
                        ...
                        Expanded(
                          child: DropdownButtonFormField<double>(
                            ...
                            items: const [
                              ...
                              DropdownMenuItem(value: 19.0, child: Text('19% (Standard)')),
                            ],
                            onChanged: (val) => setState(() => _taxRatePercent = val ?? 19.0),
                          ),
                        ),
                      ],
                    ),
              ...
              VariantMatrixEditor(
                productCode: _codeCtrl.text.isNotEmpty ? _codeCtrl.text : _nameCtrl.text,
                defaultCost: cost,
                defaultPrice: price,
