...
class StockLocations extends Table {
  ...
  TextColumn get locationType => text()(); // SHOP_FLOOR, BACK_ROOM, DAMAGED, WAREHOUSE
...
class StockMovements extends Table {
  TextColumn get id => text()();
  TextColumn get variantId => text().references(ProductVariants, #id)();
  TextColumn get fromLocationId => text().nullable().references(StockLocations, #id)();
  TextColumn get toLocationId => text().nullable().references(StockLocations, #id)();
  TextColumn get movementType => text()(); // INITIAL, PURCHASE_RECEIPT, SALE, RETURN, DAMAGE, etc.
  ...
  TextColumn get referenceId => text().nullable()(); // sale_id, po_id, return_id, count_id, etc.
  TextColumn get referenceType => text().nullable()(); // SALE, PURCHASE, RETURN, ADJUSTMENT
...
class InventoryCounts extends Table {
  ...
  TextColumn get locationId => text().references(StockLocations, #id)();
  TextColumn get status => text().withDefault(const Constant('IN_PROGRESS'))(); // IN_PROGRESS, COMPLETED, CANCELLED
  TextColumn get scope => text().withDefault(const Constant('FULL'))(); // FULL, CATEGORY, BRAND
...
class StockTransfers extends Table {
  ...
  TextColumn get status => text().withDefault(const Constant('DRAFT'))(); // DRAFT, REQUESTED, SENT, RECEIVED, CANCELLED
  TextColumn get initiatedById => text().references(Users, #id)();
