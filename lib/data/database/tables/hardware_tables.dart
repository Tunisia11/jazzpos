...
class HardwareDevices extends Table {
  ...
  TextColumn get deviceType =>
      text()(); // RECEIPT_PRINTER, LABEL_PRINTER, SCANNER, CASH_DRAWER, CUSTOMER_DISPLAY
  TextColumn get name => text()();
  TextColumn get connectionType =>
      text()(); // USB, NETWORK, SERIAL, WINDOWS_DRIVER, SIMULATED
  TextColumn get addressOrPort =>
      text()(); // IP address, COM port, USB VID/PID, or queue name
...
class PrintJobs extends Table {
  ...
  TextColumn get targetDeviceId =>
      text().nullable().references(HardwareDevices, #id)();
...
class LabelTemplates extends Table {
  ...
  TextColumn get templateType =>
      text().withDefault(const Constant('TSPL'))(); // TSPL, ZPL, ESC_POS
